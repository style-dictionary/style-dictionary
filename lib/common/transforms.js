 ...
 * @typedef {import('../../types/TokenTypes.d.ts').DimensionValue} DimensionValue
 */
 ...
 * @returns {{ value: number; unit: DimensionValue['unit'] | undefined }}
 */
export function getTokenDimensionValue(token, { usesDtcg } = { usesDtcg: false }) {
  let val = usesDtcg ? token.$value : token.value;

  /** @type {DimensionValue['unit'] | undefined} */
  let unit;
  ...
  if (!valueObj) {
    ...
    if (unitMatch) {
      unit = /** @type {DimensionValue['unit']} */ (unitMatch[0]);
...
export default {
  ...
  [transforms.sizeSp]: {
    ...
    filter: isFontSize,
    transform: function (token, _, options) {
      const nonParsed = getTokenDimensionValue(token, options);
