...
export default {
  ...
  /**
   * Turns duration tokens object-value into stringified value with unit
   *
   * ```js
   * // Matches: token.type === 'duration'
   * // Returns: "200ms"
   * ```
   *
   * @memberof Transforms
   */
  [transforms.durationStringify]: {
    type: value,
    filter: (token, options) => {
      const type = options.usesDtcg ? token.$type : token.type;
    ...
    transform: function (token, _, options) {
      const val = options.usesDtcg ? token.$value : token.value;
      if (typeof val === 'object' && val.unit && val.value !== undefined) {
