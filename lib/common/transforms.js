 ...
 * @param {PlatformConfig} platformOptions
 * @returns {string | undefined}
 */
export function getColorStringFromTokenObjectValue(tokenValue, platformOptions) {
  ...
  if (typeof tokenValue === 'object' && tokenValue !== null && 'color' in tokenValue) {
    ...
    // allow conversion to another color space via platform options
    if (platformOptions.cssShorthandsColorSpace) {
      colorJS = colorJS.to(platformOptions.cssShorthandsColorSpace);
    }
    // same for output format, must be available within that color space, defaults to undefined
    hex = colorJS.toString({ format: platformOptions.cssShorthandsColorFormat });
  }
  return hex;
}
...
export default {
  ...
  [transforms.borderCssShorthand]: {
    ...
    transform: (token, platformOptions, options) => {
      const val = options.usesDtcg ? token.$value : token.value;
      if (typeof val !== 'object') {
        // already transformed to string
        return val;
      }
      const hex = getColorStringFromTokenObjectValue(val, platformOptions);
      const { width } = val;
      ...
      return `${width ? `${width} ` : ''}${style ? `${style}` : 'none'}${hex ? ` ${hex}` : ''}`;
    },
  },
