 ...
 * @typedef {import('../../types/DesignToken.d.ts').DimensionValue} DimensionValue
 */
 ...
 * @param {Token} token
 * @param {Config} [options]
 * @returns {Record<keyof DimensionValue, any>}
 */
export function getTokenValue(token, { usesDtcg } = { usesDtcg: false }) {
  let val = usesDtcg ? token.$value : token.value;
  let unit = '';
  const valueObj = typeof val === 'object' && !Array.isArray(val) && val !== null;

  if (!valueObj && `${val}`.match(/[^0-9.-]+$/)) {
    const unitlessValue = val.replace(/([^0-9]+$)/, '');
    unit = val.replace(unitlessValue, '');
    val = unitlessValue;
  }

  return valueObj
    ? { value: val.value, unit: val.unit }
    : { value: unit ? `${val}${unit}` : val, unit: unit ? unit : undefined };
}
...
export default {
  ...
  [transforms.sizeSp]: {
    ...
    filter: isFontSize,
    transform: function (token, _, options) {
      const nonParsed = getTokenValue(token, options);
      const val = parseFloat(nonParsed.value);
      if (isNaN(val)) throwSizeError(token.name, nonParsed.value, 'sp');
  ...
  [transforms.sizeDp]: {
    ...
    transform: function (token, _, options) {
      const nonParsed = getTokenValue(token, options);
      const val = parseFloat(nonParsed.value);
      if (isNaN(val)) throwSizeError(token.name, nonParsed.value, 'dp');
  ...
  [transforms.sizeObject]: {
    ...
    transform: function (token, config, options) {
      ...
      if (isNaN(parsedVal)) throwSizeError(token.name, nonParsed.value, 'object');
  ...
  [transforms.sizeRemToSp]: {
    ...
    transform: function (token, config, options) {
      ...
      if (isNaN(parsedVal)) throwSizeError(token.name, nonParsed.value, 'sp');
  ...
  [transforms.sizeRemToDp]: {
    ...
    transform: function (token, config, options) {
      ...
      if (isNaN(parsedVal)) throwSizeError(token.name, nonParsed.value, 'dp');

      // considering the unit is a breaking-change of the transformer
      // since we can assume the consumers did use units in their token values
      // and this behaviour will now consider these units and only scale REM values
      // instead of previously making no difference between unit/less values and scale them regardlessly
      // if (nonParsed.unit && nonParsed.unit !== 'rem') return `${parsedVal.toFixed(2)}dp`;
  ...
  [transforms.sizePx]: {
    ...
    transform: function (token, _, options) {
      const nonParsed = getTokenValue(token, options);
      const parsedVal = parseFloat(nonParsed.value);
      if (isNaN(parsedVal)) throwSizeError(token.name, nonParsed.value, 'px');
  ...
  /**
   * Adds 'rem' to the end of the number. Does scale pixel values to rem if `useUnits` on the platform options in your config is set
   * by a factor of 16 (or the value of 'basePxFontSize' on the platform in your config).
   *
   * ```js
   * // Matches: token.type === 'dimension'
   * // Returns:
  ...
  [transforms.sizeRem]: {
    ...
    filter: (token, options) => isDimension(token, options) || isFontSize(token, options),
    transform: function (token, config, options) {
      const useUnits = config.options?.useUnits;
      const baseFont = getBasePxFontSize(config);
      const nonParsed = getTokenValue(token, options);

      const parsedVal = parseFloat(nonParsed.value);
      if (isNaN(parsedVal)) throwSizeError(token.name, nonParsed.value, 'rem');
      if (parsedVal === 0) return Number.isInteger(nonParsed.value) ? 0 : '0';

      // only scale pixel value if set in the platform options
      if (useUnits && nonParsed.unit === 'px') return `${parsedVal / baseFont}rem`;

      // if the dimension already has a unit (non-digit / . period character)
      if (`${nonParsed.value}`.match(/[^0-9.-]+$/)) return nonParsed.value;
  ...
  [transforms.sizeRemToPt]: {
    ...
    transform: function (token, config, options) {
      ...
      const parsedVal = parseFloat(nonParsed.value);
      const baseFont = getBasePxFontSize(config);
      if (isNaN(parsedVal)) throwSizeError(token.name, nonParsed.value, 'pt');
  ...
  [transforms.sizeComposeRemToSp]: {
    ...
    transform: function (token, config, options) {
      ...
      // and this behaviour will now consider these units and only scale REM values
      // instead of previously making no difference between unit/less values and scale them regardlessly
      // if (nonParsed.unit && !['rem', '.em'].includes(nonParsed.unit))
      //   when transformGroup 'compose' is used, the token value might already have been transformed by the `sizeComposeEm` transformer
      //   to stay backwards compatible the token value transformed to `.em` needs to be scaled similar to the REM value
      //   return `${parsedVal.toFixed(2)}.sp`;
  ...
  [transforms.sizeComposeEm]: {
    ...
    transform: function (token, _, options) {
      const nonParsed = getTokenValue(token, options);
      const parsedVal = parseFloat(nonParsed.value);
      if (isNaN(parsedVal)) throwSizeError(token.name, nonParsed.value, 'em');
  ...
  [transforms.sizeSwiftRemToCGFloat]: {
    ...
    transform: function (token, config, options) {
      ...
      const parsedVal = parseFloat(nonParsed.value);
      const baseFont = getBasePxFontSize(config);
      if (isNaN(parsedVal)) throwSizeError(token.name, nonParsed.value, 'CGFloat');

      // considering the unit is a breaking-change of the transformer
      // since we can assume the consumers did use units in their token values
      // and this behaviour will now consider these units and only scale REM values
      // instead of previously making no difference between unit/less values and scale them regardlessly
      // if (nonParsed.unit && nonParsed.unit !== 'rem') return `CGFloat(${parsedVal.toFixed(2)})`;
  ...
  [transforms.sizeRemToPx]: {
    ...
    transform: function (token, config, options) {
      ...
      // and this behaviour will now consider these units and only scale REM values
      // instead of previously making no difference between unit/less values and scale them regardlessly
      // if (nonParsed.unit && nonParsed.unit !== 'rem') return `${parsedVal.toFixed(0)}px`;
  ...
  [transforms.sizePxToRem]: {
    ...
    transform: (token, config, options) => {
      ...
      if (isNaN(parsedVal)) {
        throwSizeError(token.name, nonParsed.value, 'rem');
      }
      ...
      // and this behaviour will now consider these units and only scale REM values
      // instead of previously making no difference between unit/less values and scale them regardlessly
      // if (nonParsed.unit && nonParsed.unit === 'rem') return `${parsedVal}rem`;
  ...
  [transforms.sizeFlutterRemToDouble]: {
    ...
    transform: function (token, config, options) {
      ...
      // and this behaviour will now consider these units and only scale REM values
      // instead of previously making no difference between unit/less values and scale them regardlessly
      // if (nonParsed.unit && nonParsed.unit !== 'rem') return parseFloat(nonParsed.value).toFixed(2);

      return (parseFloat(nonParsed.value) * baseFont).toFixed(2);
    },
  },
