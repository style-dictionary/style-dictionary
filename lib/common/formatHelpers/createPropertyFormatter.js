 ...
 * @param {Formatting} [options.formatting] - Custom formatting properties that define parts of a declaration line in code. The configurable strings are: `prefix`, `indentation`, `separator`, `suffix`, `lineSeparator`, `commentStyle` and `commentPosition`. Those are used to generate a line like this: `${indentation}${prefix}${token.name}${separator} ${token.value}${suffix}`. Note: `header`, `footer`, and `fileHeaderTimestamp` are not used here — they are consumed by the `fileHeader()` format helper instead.
...
export default function createPropertyFormatter({
  ...
  // Strip fileHeader sub-object — it's only relevant to the fileHeader() helper,
  // not to token property formatting. Prevents noise in mergedOptions.
  const { fileHeader: _fh, ...cleanFormatting } = formatting;
