...
const logLevelsWithoutSilent = Object.values(logLevels).filter((level) => level !== 'silent');
 ...
 * @typedef {Map<string, Log[]>} LogMapSub
 * @typedef {Record<string, LogMapSub>} LogMap
 * @typedef {Exclude<LogLevels, typeof logLevels.silent>} LogLevelsWithoutSilent
 */
...
export class Logger {
   ...
   */
  constructor(config = {}) {
    // Create a store object for each log level (besides silent, storing silent logs makes no sense)
    /** @type {[LogLevelsWithoutSilent, LogMap][]} */
    const storeEntries = Object.values(logLevels)
      .filter((level) => level !== 'silent')
      .map((level) => [level, {}]);
    /** @type {Record<LogLevelsWithoutSilent, LogMap>} */
    this.store = /** @type {Record<LogLevelsWithoutSilent, LogMap>} */ (
      Object.fromEntries(storeEntries)
    );
    // we need a typecast above because Object.fromEntries is dumb and doesn't use the key as index signature

    // this maps string keys for logs to the store entries
    // so we can find things easily
    /** @type {Map<string, LogMapSub>} */
    this.logMap = new Map();

    /** @type {LogConfig & Required<Pick<LogConfig, 'threshold'>>} */
    this.config = {
    ...
    if (Object.keys(config).every((item) => ['warnings', 'errors', 'verbosity'].includes(item))) {
  ...
  add(type, key, toLog) {
    ...
    // happens internally first time we add a log for SD's logs but also
    // e.g. when a subclasser or user is doing their own kinds of logging in their registered hook
    if (!map && type.split('.').length === 2) {
      this.validateLogType(type, true);
      const splitType = type.split('.');
      const [parentType, subType] = splitType;
      const logCategory = this.store[/** @type {Exclude<LogLevels, 'silent'>} */ (parentType)];
  ...
  /**
   * Overridable method for getting the final message
   * based on verbosity, and the verbose / non-verbose messages.
   *
   * You could override this in an extension if you want to change the verbosity footer
   * for verbose logs.
   *
   * @protected
  ...
  log(_message, _typeOrVerbose, _type) {
    ...
    // don't log anything
    if (loggerValueMap[this.config.threshold] > loggerValueMap[level]) {
      return;
    }
   ...
   * @private
   * @param {string} type
   * @param {boolean} [mustHaveSubtype]
   */
  validateLogType(type, mustHaveSubtype = false) {
    const typeSplit = type.split('.');
    const parentType = /** @type {LogLevelsWithoutSilent} */ (typeSplit[0]);
    if (
      (mustHaveSubtype ? typeSplit.length === 2 : true) &&
      logLevelsWithoutSilent.includes(parentType)
    ) {
      return;
    }
    // This is always fatal and cannot be silenced through the Logger.
    throw new Error(
      `When adding logs, the "type" argument must be a string with one of the allowed log level prefixes: [${logLevelsWithoutSilent.join(', ')}]. We got ${parentType} instead.`,
    );
  }
