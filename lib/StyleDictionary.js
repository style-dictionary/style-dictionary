...
import { logErrorLevels, logVerbosityLevels, logWarningLevels } from './enums/index.js';
...
export default class StyleDictionary extends Register {
  ...
  async _exportPlatform(platform) {
    ...
    if (refErrorCount > 0) {
      ...
      if (this.log.errors?.brokenReferences === logThrow) {
        throw new Error(err);
    ...
    const transformErrorCount = this.logger.count('errors.transforms');
    if (transformErrorCount > 0) {
      const flushed = /** @type {Map<string, TransformError[]>} */ (
        this.logger.flush('errors.transforms')
      );
      const errors = [...flushed]
        .flatMap(([, v]) => v)
        .map((v) => v.message)
        .join('\n');
      ...
      if (this.log.errors?.transforms === logThrow) {
        throw new Error(err);
      } else if (this.log.verbosity !== silent) {
        console.error(err);
      }
