...
const { console: logConsole } = logErrorLevels;
...
export default class StyleDictionary extends Register {
  ...
  async extend(config = this.config, opts = {}) {
    ...
    this.log = {
      ...
      verbosity: opts.verbosity ?? options.log?.verbosity ?? this.log.verbosity ?? defaultVerbosity,
      warnings: opts.warnings ?? options.log?.warnings ?? this.log.warnings ?? warn,
    };
    this.logger = options.logger ?? new Logger(this.log);
    this.options = {
      ...options,
      log: this.log,
      logger: this.logger,
  ...
  async _exportPlatform(platform) {
    ...
    const refErrorCount = GroupMessages.count(PROPERTY_REFERENCE_WARNINGS);
    if (refErrorCount > 0) {
      const errors = GroupMessages.flush(PROPERTY_REFERENCE_WARNINGS).join('\n');
      let err = `\nReference Errors:\nSome token references (${refErrorCount}) could not be found.\n`;
      ...
      if (this.log.errors?.brokenReferences !== logConsole) {
        throw new Error(err);
    ...
    const transformErrors = this.logger?.store.error.transforms;
    const transformErrorCount = transformErrors?.size;
    if (transformErrorCount && transformErrorCount > 0) {
      const errors = [...transformErrors]
        .flatMap(([, v]) => v)
        .map((v) => v.message)
        .join('\n');
      let err = `Some token transformations (${transformErrorCount}) could not be applied correctly.\n`;
      let errVerbose = `${err}\n${errors}\n`;
      this.logger?.log(err, errVerbose, logBuiltinKeys['error.transforms']);
    }
