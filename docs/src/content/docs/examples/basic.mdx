...
2. The build system resolves references to other design tokens. `{dimensions.4}` gets resolved properly.
3. The build system handles references to token values in other files as well as you can see in `tokens/object-values.json`, which references tokens from `tokens/colors.json` and `tokens/dimensions.json`.

Now let's make a change and see how that affects things. Open up `tokens/colors.json` and change `"#000000"` to `"#111111"`. After you make that change, save the file and re-run the build command `style-dictionary build`. Open up the build files and take a look.
