...
Use PlatformConfig option `cssShorthandsColorSpace` & `cssShorthandsColorFormat` to determine the format to which you want DTCG color definitions to be stringified.
This is using ColorJS conversions.

Example: if your source token is in lab colorspace and you want hex output, you'll have to convert to srgb colorspace first.
...
```js
{
  platforms: {
    css: {
      files: [{}],
      cssShorthandsColorSpace: 'srgb';
      cssShorthandsColorFormat: 'hex';
    }
  }
}
```
