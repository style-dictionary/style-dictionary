...
Given that Style Dictionary is written in type-annotated JavaScript (via JSDocs) and that TypeScript enums are notoriously user-unfriendly, we're exporting enums as type-annotated JavaScript constants (enum-like objects). This provides the same kind of type-safety, is easier to use because you can use the matching string, and it can also be used in JavaScript projects.

These enum-like objects are used internally within Style Dictionary. You can also use them in your own configurations, whether you are working with TypeScript or JavaScript.
...
#### Log Broken Reference Levels <Badge text="Deprecated" variant="danger" />

Use logErrorLevels instead

```javascript
// enums/logging/logBrokenReferenceLevels.js
...
```javascript
// enums/logging/logBuiltinKeys.js
export const logBuiltinKeys = {
  'throw.references': 'throw.references',
  'error.transforms': 'error.transforms',
  'warn.collisions': 'warn.collisions',
  'warn.nameCollisions': 'warn.nameCollisions',
};
```
...
```javascript
// enums/logging/logErrorLevels.js
...
```javascript
// enums/logging/logVerbosityLevels.js
export const logVerbosityLevels = {
  default: 'default',
  verbose: 'verbose',
  /** @deprecated  use log.threshold -> 'silent' instead */
  silent: 'silent',
};
```
...
```javascript
// enums/logging/logWarningLevels.js
